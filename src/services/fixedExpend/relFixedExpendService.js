...
var fixedExpendService = {
    ...
    getFixedExpendsAndValues: function(user_id, month) {
        ...
        let query = `select rel_fixed_expend.id_fe,rel_fixed_expend.user_id,name,value,month,dailyUse from rel_fixed_expend
                     inner join expend 
    ...
    getFixedExpendsAndValuesSpecial: function(user_id, month) {
        ...
        inner join special_expend  on rel_fixed_expend.id_fe = special_expend.id_fe where month <= ? and special_expend.state_code = ? and active = ? and special = 1 and rel_fixed_expend.user_id = ? and (special_expend.id_fe,month) 
        IN  (SELECT special_expend.id_fe, MAX(month) FROM special_expend where month < ? GROUP BY special_expend.id_fe);`;
        return pool.query(query, rows);
    },

    setFixedExpend: function(user_id, special) {
        const query = `insert into rel_fixed_expend(user_id,creation_date,state,active,special) values(?,?,?,?,?)`;
        state = 1;
        active = 1;
        const timeElapsed = Date.now();
        const creation_date = new Date(timeElapsed).toISOString();
        rows = [user_id, creation_date, state, active, special];
        return pool.query(query, rows);
    },
