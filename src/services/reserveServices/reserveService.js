...
var reserveService = {
    ...
    getReserves: function(user_id) {
        let query;
        let rows = [user_id];
        query = 'SELECT * FROM reserve WHERE (user_id) = ? AND (state)=1';
    ...
    getReserveByMonth: function(user_id, month) {
        ...
        query = 'SELECT * FROM reserve WHERE (user_id) = ? AND (month) = ? AND (state)=1 ';
        return pool.query(query, rows);
    },

    setReserve: function(user_id, name, value, available, month, id_fixed_reserve) {
        const query = `insert into reserve(user_id,name,value,month,state,id_fixed_reserve) values(?,?,?,?,?,?,?)`;
        state = 1;
    ...
    setMultipleReserves: function(additional) {
        const query = `insert into reserve(user_id,name,value,month,state,id_fixed_reserve) values ${additional}`;
        rows = [];
        return pool.query(query, rows);
    },
    
    updateReserve: function(id, user_id, name, value) {
        let query = `UPDATE reserve
                SET 
                name = ?, 
                value = ?
                WHERE id = ?
