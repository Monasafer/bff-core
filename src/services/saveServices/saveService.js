...
var saveService = {
    ...
    setSave : function(user_id, desc, value){
        if(Validate(desc)==true){
            const query = `insert into save(desc, value, user_id, creation_date, state_code) values(?,?,?,?,1)`;
    ...
    updateSave : function(user_id, saveId, desc, value){
        if(Validate(desc)==true){
            let query =  `UPDATE save
                ...
                WHERE id = ? 
                AND user_id = ?`;
        rows = [desc, value, saveId, user_id];
        return pool.query(query, rows);
